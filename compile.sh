...
nasm -i boot -f bin boot/bootsect.asm -o build/boot/bootsect.bin
nasm -i boot -f bin boot/padding.asm -o build/boot/padding.bin
nasm -i boot -f elf boot/kernel_entry.asm -o build/boot/kernel_entry.o
nasm -i boot -f elf cpu/interrupt.asm -o build/cpu/interrupt.o
...
x86_64-elf-ld -o build/kernel/kernel.bin -Ttext 0x1000 --oformat binary \
...
cat build/boot/bootsect.bin >> build/os-image.bin
